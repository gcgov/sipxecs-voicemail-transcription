...
func cloudstorage(bucket string, path string, filename string, method string) {
	...
	var object string = filename

	switch method {
	case "write":
		if err := write(client, bucket, object, path); err != nil {
...
func write(client *storage.Client, bucket, object string, path string) error {
	ctx := context.Background()
	// [START upload_file]
	f, err := os.Open(path)
	if err != nil {
